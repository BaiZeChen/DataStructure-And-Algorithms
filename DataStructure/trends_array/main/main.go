...
func (s *Slice) init(capacity int) *Slice {
	...
	s.size = (*int)(unsafe.Pointer(uintptr(unsafe.Pointer(&s.arr)) + uintptr(8)))
	return s
}
...
func (s *Slice) Append(elems ...int) {
	capacity := cap(s.arr)
	addLength := len(elems)
	newSize := addLength + *s.size

	// 得扩容了
	if newSize > capacity {
		s.expansion(capacity, newSize)
	}
...
func (s *Slice) expansion(oldCap, size int) {
	var newCap = 0
	for newCap < size {
		if newCap == 0 {
			newCap = oldCap * 2
